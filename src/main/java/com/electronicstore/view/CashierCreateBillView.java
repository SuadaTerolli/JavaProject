...
public class CashierCreateBillView extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Label sector = new Label("Sector: " + getSectorForUser(loggedInUser,loggedInUser.getId(),filePath)); // Assuming sector is derived from the user ID
    ...
    private String getSectorForUser(User user,int userId,String filePath) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length >= 10) { // Ensure the sectorId column exists
                    String username = parts[2].trim();
                    String sectorNameValue = parts[9].trim(); // Get the sectorId column

                    // Check if the username matches the given user
